...
const productController = {
  async list(req, res, next) {
    try {
      const result = await Product.findAll(req.user.id, req.query);
  ...
  async update(req, res, next) {
    try {
      const product = await Product.update(req.params.id, req.user.id, req.body);
      if (!product) return res.status(404).json({ success: false, message: 'Produk tidak ditemukan.' });
  ...
  async delete(req, res, next) {
    try {
      const result = await Product.delete(req.params.id, req.user.id);
