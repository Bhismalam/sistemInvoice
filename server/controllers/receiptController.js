...
const receiptController = {
  ...
  async getAll(req, res, next) {
    try {
      ...
      const result = await Receipt.findAll(req.user.id, { transaction_type, document_id, search, sort, order, page, limit });
  ...
  async getById(req, res, next) {
    try {
      const receipt = await Receipt.findById(req.params.id, req.user.id);
  ...
  async delete(req, res, next) {
    try {
      const result = await Receipt.delete(req.params.id, req.user.id);
