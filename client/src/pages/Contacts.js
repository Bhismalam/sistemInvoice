...
export function renderContacts(container) {
  ...
  async function load() {
    try {
      ...
      list.querySelectorAll('.del-c').forEach(b => b.addEventListener('click', async (e) => { 
        e.stopPropagation();
        if (await showConfirm('Hapus kontak?')) { 
          await api(`/contacts/${b.dataset.id}`, {method:'DELETE'}); 
          showToast('Kontak dihapus','success'); 
          load(); 
        }
      }));

      list.querySelectorAll('.edit-c').forEach(b => b.addEventListener('click', async (e) => {
        e.stopPropagation();
        try {
          const res = await api(`/contacts/${b.dataset.id}`);
          showContactModal(res.data);
        } catch (err) {
  ...
  function showContactModal(data) {
      ...
      <div class="modal-overlay" id="modal-bg"><div class="modal-content">
        ...
        <form id="contact-form">
          <div class="form-group"><label class="form-label">Tipe</label>
            <select class="form-select" id="c-type" required>
              <option value="customer" ${data && data.type === 'customer' ? 'selected' : ''}>Pelanggan</option>
              <option value="supplier" ${data && data.type === 'supplier' ? 'selected' : ''}>Supplier</option>
            </select>
          </div>
          <div class="form-group"><label class="form-label">Nama</label><input class="form-input" id="c-name" required placeholder="Nama perusahaan/kontak" value="${data ? data.name || '' : ''}" /></div>
          <div class="form-group"><label class="form-label">Email</label><input type="email" class="form-input" id="c-email" placeholder="email@example.com" value="${data ? data.email || '' : ''}" /></div>
          <div class="form-group"><label class="form-label">Telepon</label><input class="form-input" id="c-phone" placeholder="08xxxxxxxxxx" value="${data ? data.phone || '' : ''}" /></div>
          <div class="form-group"><label class="form-label">Alamat</label><textarea class="form-input form-textarea" id="c-address" placeholder="Alamat lengkap">${data ? data.address || '' : ''}</textarea></div>
    ...
    document.getElementById('contact-form').addEventListener('submit', async e => {
      ...
      try {
        ...
        if (data) {
          // Edit mode
          await api(`/contacts/${data.id}`, { method: 'PUT', body: bodyData });
          showToast('Kontak berhasil diperbarui!', 'success');
        } else {
          // Add mode
          await api('/contacts', { method: 'POST', body: bodyData });
