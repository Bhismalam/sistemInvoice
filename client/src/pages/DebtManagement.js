...
export function renderDebtManagement(container, routeParams = {}) {
  ...
  page.innerHTML = `<div class="animate-slide-up">
    <div class="page-header">
      <div>
        <h1 class="page-title">💳 Manajemen Hutang & Piutang</h1>
    ...
    try {
      ...
      if (allItems.length === 0) {
        el.innerHTML = `<div class="empty-state">
          <div class="empty-state__icon">${type === 'purchase' ? '📤' : '📥'}</div>
        ...
        ${data.overdue.length > 0 ? `
          <div class="debt-section">
            <h4 class="debt-section__title debt-section__title--danger">
              <span>⚠️</span> Jatuh Tempo (${data.overdue.length})
              ...
              <tbody>
                ${data.overdue.map(d => `
                  <tr class="row-overdue">
                    ...
                    <td>
                      <button class="btn btn-primary btn-sm quick-pay" data-id="${d.id}" data-type="${typeLabel}">💰 Bayar</button>
                    </td>
                  </tr>
                `).join('')}
              </tbody>
        ...
        ${data.upcoming.length > 0 ? `
          <div class="debt-section" style="margin-top:var(--space-xl)">
            <h4 class="debt-section__title">
              <span>📅</span> Mendatang (${data.upcoming.length})
              ...
              <tbody>
                ${data.upcoming.map(d => {
                  ...
                  <tr class="${daysLeft <= 3 ? 'row-due-soon' : ''}">
                    ...
                    <td>
                      <button class="btn btn-ghost btn-sm quick-pay" data-id="${d.id}" data-type="${typeLabel}">💰</button>
                    </td>
                  </tr>
                `}).join('')}
              </tbody>
    ...
    try {
      ...
      if (upcoming.length > 0) {
        html += `<div class="reminder-section">
          <h4 class="debt-section__title debt-section__title--warning">
            <span>🔔</span> Pengingat Mendatang (${upcoming.length})
          </h4>
          <div class="reminder-list">
            ${upcoming.map(r => {
              const typeLabel = r.transaction_type === 'purchase' ? 'purchases' : 'sales';
              return `<div class="reminder-card">
                <div class="reminder-card__icon">${r.reminder_type === 'on_due' ? '⏰' : r.reminder_type === 'after_due' ? '⚠️' : '🔔'}</div>
      ...
      if (reminders.length > 0) {
        html += `<div class="reminder-section" style="margin-top:var(--space-xl)">
          <h4 class="debt-section__title">
            <span>📋</span> Semua Pengingat (${reminders.length})
          </h4>
          <table class="data-table">
            ...
            <tbody>
              ${reminders.map(r => {
                ...
                return `<tr>
                  ...
                  <td><button class="btn btn-ghost btn-sm reminder-delete" data-id="${r.id}" style="color:var(--danger)">🗑️</button></td>
                </tr>`;
              }).join('')}
            </tbody>
          </table>
