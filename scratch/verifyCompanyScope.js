...
const { initDB } = require('../server/config/database');
const { User } = require('../server/models/User');
const { Product } = require('../server/models/Product');
const { Contact } = require('../server/models/Contact');
const Document = require('../server/models/Document');
const Receipt = require('../server/models/Receipt');
const PaymentReminder = require('../server/models/PaymentReminder');
...
async function runVerification() {
  ...
  const staffProducts = await Product.findAll(staffId, {}, companyId);
  ...
  const staffContacts = await Contact.findAll(staffId, {}, companyId);
  ...
  const updatedProduct = await Product.findById(product._id, ownerId, companyId);
  ...
  await Document.Document.deleteOne({ _id: invoice._id });
